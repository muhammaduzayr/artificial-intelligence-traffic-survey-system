...
sys.path.insert(0, str(ROOT / "aitss"))
...
def parse_args():
    ...
    videos = sorted((ROOT / "aitss").glob("*.mp4"))
...
def main():
    ...
    if args.video is None:
        raise FileNotFoundError(f"No .mp4 survey video found in {ROOT / 'aitss'}")
    ...
    model_path = args.model.expanduser().resolve() if args.model else ROOT / config.MODEL_PATH
